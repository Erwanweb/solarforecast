...
"""
<plugin key="RL-SOLARFORECAST" name="Ronelabs - Solar Forecast plugin" author="Ronelabs" version="0.0.2" externallink="https://github.com/Erwanweb/solarforecast.git">
      <description>
        <h2>Ronelabs - Solar Forecast plugin for domoticz</h2><br/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        devicecreated = []
        if 1 not in Devices:
            Domoticz.Device(Name="Total Today", Unit=1, Type=243, Subtype=31, Used=1).Create()
            devicecreated.append(deviceparam(1, 0, "0"))  # default is 0 Kwh forecast
        if 2 not in Devices:
            Domoticz.Device(Name="Total Tomorrow", Unit=1, Type=243, Subtype=31, Used=1).Create()
            devicecreated.append(deviceparam(1, 0, "0"))  # default is 0 Kwh forecast
