...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if self.ForecastRequest <= now:
            self.ForecastRequest = datetime.now() + timedelta(minutes=15) # make make a Call every 15 minutes

            # Set location using domoticz param
            latlon = DomoticzAPI("type=command&param=getsettings")
            if latlon:
                self.lat = str(latlon['Location']['Latitude'])
                LATITUDE = str(latlon['Location']['Latitude'])
                self.lon = str(latlon['Location']['Longitude'])
                LONGITUDE = str(latlon['Location']['Longitude'])
    ...
    def CheckForecast(self):
        ...
        """
        with open('/home/tools/Solarforecat/PVPlant.py', 'w') as f:
        f.write(f"# PV Pant variables\n")
        f.write(f"#\n")
        ...
        try:
            ...
            Domoticz.Debug(f"Calling forecast API {url}")
            ...
            """
            with open('/home/domoticz/plugins/solarforecast/solar_forecast_variables.py', 'w') as f:
            ...
            if 'result' in data and 'watt_hours_day' in data['result']:  # Power per hour part
                ...
                for entry in data['result']['watt_hours_day']:
                    ...
                    hourly_data[variable_name] = hourly_watts
                forecastreceived = False
            else:
...
"""
            ...
            with open('/home/domoticz/plugins/solarforecast/solar_forecast.json', 'w') as outfile:
                json.dump(hourly_data, outfile, indent=4)

            Domoticz.Debug("Hourly data has been written to 'solar_forecast.json'")
